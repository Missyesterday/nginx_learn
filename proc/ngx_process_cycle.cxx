...
//父进程的循环,父进程创建worker子进程,并返回这个函数的循环
...
{    
    ...
    //建议fork()子进程时使用这种写法，防止信号的干扰；
    //执行这个 函数的时候, 不要让这些信号干扰我
    ...
    //设置，此时无法接受的信号；阻塞期间，发过来的上述信号，多个会被合并为一个，暂存着，等你放开信号屏蔽后才能收到这些信号。。。
    ...
    if(size < 1000) //长度小于这个，才设置标题, (怎么可能标题大于1000)
    {
        char title[1000] = {0};
        strcpy(title,(const char *)master_process); //"master process"
        strcat(title," ");  //title最后跟一个空格分开一些，清晰    //"master process "
        for (i = 0; i < g_os_argc; i++)         //"master process ./nginx" + 命令行输入的其他参数
        {
    ...
    sigemptyset(&set); //信号屏蔽字为空，表示不屏蔽任何信号, 对信号的阻塞没有影响,因为没有调用sigprocmask函数
