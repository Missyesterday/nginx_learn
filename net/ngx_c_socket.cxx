...
bool CSocekt::ngx_open_listening_sockets()
{
    ...
    for(int i = 0; i < m_ListenPortCount; i++) //要监听这多少个端口,就需要循环几次
    {        
        ...
        serv_addr.sin_port = htons((in_port_t)iport);   //in_port_t其实就是uint16_t, htons就是主机序转网络序, 4个字节
        ...
        //可以，放到列表里来
        lpngx_listening_t p_listensocketitem = new ngx_listening_t; //千万不要写错，注意前边类型是指针，后边类型是一个结构体, 里面是listenfd和对应的端口
