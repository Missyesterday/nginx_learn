...
void CSocket::initconnection()
{
    ...
    for(int i = 0; i < m_worker_connections; ++i) //先创建这么多个连接，后续不够再增加
    {
        ...
        p_Conn = new(p_Conn) ngx_connection_t();  //定位new，释放则显式调用p_Conn->~ngx_connection_t();		定位new是用来在p_Conn首地址调用构造函数
        p_Conn->GetOneToUse();
        ...
        if(pSocketObj->m_totol_recyconnection_n > 0)
        {
            ...
            for(; pos != posend; )
            {
                ...
                //流程走到这里，表示可以释放
                    ...
                    pos = pSocketObj->m_recyconnectionList.erase(pos);   //迭代器已经失效，但pos所指内容在p_Conn里保存着呢
