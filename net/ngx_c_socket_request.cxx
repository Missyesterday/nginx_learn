...
/*     unsigned char buf[10] = {0};
    ...
    //每次只接收两个字节, 客户端发送老多数据在服务器的收缓冲区存着, 但是ET模式可能只会收取一部分
    do
    {
        int n = recv(pConn->fd, buf, 2, 0);
        if(n == -1 && errno == EAGAIN)
            break;//数据收完了
        if(n == 0) //客户端断开连接
            break;
 ...
 */    
     ...
     //收包，注意我们用的第二个和第三个参数，我们用的始终是这两个参数，因此我们必须保证 pConn->precvbuf指向正确的收包位置，保证c->irecvlen指向正确的收包宽度
...
//发送数据专用函数，返回本次发送的字节数, 参数: 连接池类型, 发送首地址, 发送长度
//返回 > 0，成功发送了一些字节
//=0，估计对方断了
...
//设置数据发送时的写处理函数,当数据可写时epoll通知我们，我们在 int CSocket::ngx_epoll_process_events(int timer)  中调用此函数
...
{      
    ...
    if(sendsize > 0 && sendsize == pConn->isendlen) //成功发送完毕，做个通知是可以的；
    {
        ...
        if(ngx_epoll_oper_event(
                ...
                1,                  //对于事件类型为增加的，EPOLL_CTL_MOD需要这个参数, 0：增加   1：去掉某个事件标记 2：完全覆盖
                pConn               //连接池中的连接
                ) == -1)
        {
