...
ngx_signal_t  signals[] = {
    ...
    { SIGSYS,    "SIGSYS, SIG_IGN",  NULL               },        //我们想忽略这个信号，SIGSYS表示收到了一个无效系统调用，如果我们不忽略，进程会被操作系统杀死，--标识31
                                                                  ...
                                                                  //在ngx_init_signal()函数会把NULL的handler设置为 SIG_IGN
    //...日后根据需要再继续增加
...
int ngx_init_signals()
{
    ...
    for (sig = signals; sig->signo != 0; sig++)  //将signo ==0作为一个标记，因为信号的编号都不为0,同时还是结构体数组中最后一个元素
    {        
        //我们注意，现在要把一堆信息往 变量sa对应的结构里添加 .
        ...
        if (sig->handler)  //如果信号处理函数不为空，这当然表示我要定义自己的信号处理函数
        {
            ...
            sa.sa_flags = SA_SIGINFO;        //sa_flags：int型，指定信号的一些选项，设置了该标记(SA_SIGINFO)，就表示信号附带的参数可以被传递到信号处理函数中
                                                //如果想让sa.sa_sigaction指定的信号处理程序(函数)生效，你就把sa_flags设定为SA_SIGINFO
        }
        else
        {
            //sa_handler和sa_sigaciton都是信号处理函数, 两个只有一个有效,后者参数更为详细
        ...
        sigemptyset(&sa.sa_mask);   //比如处理某个信号比如SIGUSR1信号时不希望收到SIGUSR2信号，那咱们就可以用诸如sigaddset(&sa.sa_mask,SIGUSR2);这样的语句针对信号为SIGUSR1时做处理
        ...
        if (sigaction(sig->signo, &sa, NULL) == -1) //参数1：要操作的信号
                                                     //参数2：主要就是那个信号处理函数以及执行信号处理函数时候要屏蔽的信号等等内容
                                                      //参数3：返回以往的对信号的处理方式【跟sigprocmask()函数边的第三个参数是一样的】，跟参数2同一个类型，我们这里不需要这个东西，所以直接设置为NULL；
        {   
