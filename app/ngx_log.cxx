...
//调用格式比如：ngx_log_stderr(0, "invalid option: \"%s\",%d", "testinfo",123);
 /* 
    ...
    */
//这个函数往屏幕上打印信息!说明这个错误特别紧急,需要终端人员立即知道
//功能和调用方法类似于printf, 相当于写了一个printf
void ngx_log_stderr(int err, const char *fmt, ...)
{    
    va_list args;                        //创建一个va_list类型变量
    u_char  errstr[NGX_MAX_ERROR_STR + 1]; //2048  ngxin源码中没有+1
    u_char  *p,*last;

    //清空, nginx没有这行代码
    memset(errstr,0,sizeof(errstr));     

    //last指向整个buffer最后【指向最后一个有效位置的后面也就是非有效位】，作为一个标记，防止输出内容超过这么长,
    last = errstr + NGX_MAX_ERROR_STR;       
    //其实我认为这有问题，所以才在上边errstr[NGX_MAX_ERROR_STR+1]; 给加了1
    //比如你定义 char tmp[2]; 你如果last = tmp+2，那么last实际指向了tmp[2]，而tmp[2]在使用中是无效的
                                                
    //p 返回的是 "nginx: "的下一个位置, 也就是能直接写入的位置
    //ngx_cpymem是一个宏函数, 作用就是拷贝字符到errstr中,返回第7个位置
    p = ngx_cpymem(errstr, "nginx: ", 7);  
    
    va_start(args, fmt); //使args指向起始的参数,所以可变参数的第一个参数实际上是fmt, 后面才是...
    
    //ngx_vslprintf(), 才是真正干活的函数, 定义在ngx_printf.cxx
...
//    日过定向为标准错误，则直接往屏幕上写日志【例如日志文件打不开，则会直接定位到标准错误，此时日志就打印到屏幕上，参考ngx_log_init()】
...
// 例如调用, ngx_log_error_core(5,8,"这个XXX工作的有问题,显示的结果是=%s","YYYY");
void ngx_log_error_core(int level,  int err, const char *fmt, ...)
{
    u_char  *last;
    u_char  errstr[NGX_MAX_ERROR_STR+1];   //这个+1nginx没有，本函数可以参考ngx_log_stderr()函数的写法；
    ...
    ngx_slprintf(strcurrtime,  
                    ...
                    tm.tm_min, tm.tm_sec);
    p = ngx_cpymem(errstr,strcurrtime,strlen((const char *)strcurrtime));  //日期增加进来，得到形如：     2023/01/08 20:26:07
    p = ngx_slprintf(p, last, " [%s] ", err_levels[level]);                //日志级别增加进来，得到形如：  2023/01/08 20:26:07 [crit] 
    
    //支持%P格式，进程id增加进来，得到形如：   2023/01/08 20:50:15 [crit] 2037:
    ...
    *p++ = '\n'; //增加个换行符       
    ...
    //一个while(1)的循环, 这么写代码更方便：随时可以把流程弄到while后边去；
    ssize_t   n;
    while(1) 
    {        
...
void ngx_log_init()
{
    ...
    if (ngx_log.fd == -1)  //如果有错误，则直接定位到 标准错误上去 
    {
        ...
        ngx_log.fd = STDERR_FILENO; //直接定位到标准错误(屏幕)
    } 
    return;
}
